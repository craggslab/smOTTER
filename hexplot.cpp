...
namespace HexPlot {
    ...
    HexPlot::HexPlot(QQuickItem *parent)
        : QQuickPaintedItem (parent),
          m_nBinsX(20),
          m_nBinsY(20),
          m_data(),
          m_bins(std::vector<quint32>(m_nBinsX * m_nBinsY)),
          m_scaleMax(m_initialScaleMax)
    {
    }

    QString HexPlot::name() const { return m_name; }
    void HexPlot::setName(const QString &name) { m_name = name; }

    QColor HexPlot::color() const { return m_color; }
    void HexPlot::setColor(const QColor &color) { m_color = color; }

    quint32 HexPlot::nBinsX() const { return m_nBinsX; }
    void HexPlot::setNBinsX(quint32 bins) {
        m_nBinsX = bins;
        m_scaleMax = m_initialScaleMax;
        m_bins.clear();
        m_bins.resize(m_nBinsX * m_nBinsY);
        updateBins(m_data.begin(), m_data.end());
    }

    quint32 HexPlot::nBinsY() const { return m_nBinsY; }
    void HexPlot::setNBinsY(quint32 bins) {
        m_nBinsY = bins;
        m_scaleMax = m_initialScaleMax;
        m_bins.clear();
        m_bins.resize(m_nBinsX * m_nBinsY);
    ...
    void HexPlot::drawHex(QPointF pos, qreal R, QPainter *painter, quint32 value, quint32 max)
    {
        ...
        for (int i = 0; i < 6; i++)
        points[i] = pos + R*2.0*points[i];

        painter->drawConvexPolygon(points, 6);
    }

    void HexPlot::draw1DHisto(QPainter *painter, const QRectF& plotArea, bool horizontal)
    {
        painter->save();
    ...
    void HexPlot::paint(QPainter *painter)
    {
        ...
        draw1DHisto(painter, plotArea, true); draw1DHisto(painter, plotArea, false);
    }

    void HexPlot::drawScalebar(QPainter *painter, const QRectF &plotArea)
    {
        painter->save();
    ...
    void HexPlot::drawAxes(QPainter *painter, const QRectF &plotArea)
    {
        painter->save();

        QFont font;
        font.setPixelSize(m_layout.axes.textHeight);
        painter->setFont(font);
        ...
        drawTextRotated(painter,
                m_layout.axes.getTickTextPos<Orientation::Vertical>(plotArea, i),
                QString::number(i / 100.0), -90.0);
        }

        font.setBold(true);
        painter->setFont(font);
        painter->drawText(m_layout.axes.getLabelPos<Orientation::Horizontal>(plotArea),
                  Qt::AlignCenter, m_layout.axes.x_label);


        drawTextRotated(painter,
                m_layout.axes.getLabelPos<Orientation::Vertical>(plotArea),
                m_layout.axes.y_label, -90.0);

        painter->restore();
    }

    void HexPlot::drawTextRotated(QPainter *painter, QRectF textRect, const QString &str, qreal rot)
    {
    ...
    void HexPlot::addData(std::list<QPointF> &data)
    {
        auto start = m_data.empty() ? std::nullopt : std::make_optional(std::prev(m_data.end()));
        m_data.splice(m_data.end(), data);

        updateBins(start.value_or(m_data.begin()), m_data.end());
    }

    void HexPlot::clearData()
    {
        for (auto i = 0u; i < m_bins.size(); i++)
        m_bins[i] = 0;
        m_scaleMax = m_initialScaleMax;
    ...
    void HexPlot::updateBins(std::list<QPointF>::iterator begin, std::list<QPointF>::iterator end)
    {
        std::for_each(begin, end,
          [this](const QPointF& val) {
            ...
            m_bins[idx]++;
            m_scaleMax = std::max(m_scaleMax, m_bins[idx]);
        } );

        update();
    }

    void HexPlot::addPoints() {
       auto distX = std::normal_distribution<qreal>(0.5, 0.15);
       auto distY = std::normal_distribution<qreal>(0.5, 0.15);
       auto list = std::list<QPointF>();

       for (int i = 0; i < 5; i++)
       {
           auto x = std::max(1e-10, std::min(0.99999999, distX(rnd) ));
           auto y = std::max(1e-10, std::min(0.99999999, distY(rnd) ));
           list.push_back({ x, y });
       }
